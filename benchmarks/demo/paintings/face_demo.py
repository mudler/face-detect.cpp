...
recognition, age/gender) on a famous PUBLIC-DOMAIN PAINTING (the Mona Lisa), and
the numbers on screen are the REAL measured CLI outputs, parsed live from the
JSON the binary prints. The age + gender are the model reading a painting.

It runs the CLI for real (detect / analyze / embed, then verify the same painting
against itself -> SAME and against a different painting -> DIFFERENT), parses the
JSON, and draws a single polished still on the dark #0d1117 palette matching the
locate-anything.cpp demo look. No em-dashes in rendered text.
...
def gather(cli, model, fixtures=None):
    """Real CLI runs on public-domain paintings. The hero is the Mona Lisa; the
    verify beat compares two DIFFERENT paintings (real cosine distance) and the
    same painting against itself, so the SAME / DIFFERENT verdicts are honest."""
    hero = str(ART_DIR / "mona_lisa.jpg")
    other = str(ART_DIR / "pearl_earring.jpg")
    det = run_json(cli, ["detect", "--model", model, "--input", hero, "--json"])
    ana = run_json(cli, ["analyze", "--model", model, "--input", hero, "--json"])
    emb = run_json(cli, ["embed", "--model", model, "--input", hero, "--json"])
    vsame = run_json(cli, ["verify", "--model", model, "--a", hero, "--b", hero])
    vdiff = run_json(cli, ["verify", "--model", model, "--a", hero, "--b", other])
    return {
        "img": hero, "det": det, "ana": ana, "emb": emb,
        "vsame": vsame, "vdiff": vdiff,
        "hero_label": "Mona Lisa  ·  Leonardo da Vinci",
    }
...
def draw_photo(cv, data, rect):
    ...
    # caption under photo: the artwork, not "real photo"
    fc = font(14, False)
    cap = data.get("hero_label", "Mona Lisa  ·  Leonardo da Vinci")
...
def render(data, out):
    ...
    sub = "recognizing the masters: face recognition on the great paintings. one binary, no Python, bit-exact vs InsightFace"
    ...
    rw = W - rx - pad
    ry = 140
    d.text((rx, ry), "ONE PAINTING, ONE BINARY, FULL INSIGHTFACE PIPELINE",
    ...
    # ---- verify / 1:N identity ----
    d.text((rx, ry), "VERIFY  (cosine distance, lower = same face)",
           font=font(13, True), fill=DIMMER)
    ry += 26
    verify_row(d, rx, ry, rw, "Mona Lisa vs Mona Lisa  (same painting)",
               data["vsame"]["distance"], data["vsame"]["verified"])
    ry += 56
    verify_row(d, rx, ry, rw, "Mona Lisa vs Girl with a Pearl Earring  (two paintings)",
               data["vdiff"]["distance"], data["vdiff"]["verified"])
    ry += 56

    note = "age + gender are the model reading a painting. GGUF weights, parity verified vs InsightFace ONNX."
    d.text((rx, ry + 6), note, font=font(13, False), fill=DIM)

    # ---- footer: logo + links ----
    logo = Image.open(HERE.parent.parent.parent / "assets" / "localai_logo.png").convert("RGBA")
...
def main():
    ...
    ap.add_argument("--cli", default=str(HERE.parent.parent.parent / "build-cpu/examples/cli/facedetect-cli"))
    ap.add_argument("--model", default=str(HERE.parent.parent.parent / "models/buffalo_l-f16.gguf"))
    ap.add_argument("--fixtures", default=str(HERE.parent.parent.parent / "tests/fixtures"))
    ap.add_argument("--out", default=str(HERE.parent.parent / "media" / "face_demo_paintings.png"))
    a = ap.parse_args()
