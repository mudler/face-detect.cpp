...
~10 REAL faces, and the numbers on screen are the REAL CLI outputs parsed live
from the JSON the binary prints.

The faces come from the in-repo real fixtures only:
  * tests/fixtures/multi.jpg  - a real group photo; we detect ~11 faces and crop
    the cleanest ones (real boxes + 5 landmarks transformed into the crop frame).
  * tests/fixtures/face_{a,b,c}.jpg - three crisp real portraits.
...
def gather(cli, model, fixtures):
    fx = Path(fixtures)
    multi = str(fx / "multi.jpg")
    det = run_json(cli, ["detect", "--model", model, "--input", multi])
    faces = det["faces"]
    mimg = Image.open(multi).convert("RGB")

    subjects = []

    def add_from_crop(src_img, box, lms, label):
        crop, map_pt = _square_crop(src_img, box)
        tmp = tempfile.NamedTemporaryFile(suffix=".png", delete=False)
        crop.save(tmp.name)
        try:
            ana = run_json(cli, ["analyze", "--model", model, "--input", tmp.name])
            emb = run_json(cli, ["embed", "--model", model, "--input", tmp.name])
        finally:
            os.unlink(tmp.name)
        if not ana["faces"]:
            return False
    ...
    # selected group faces (cleanest crops that analyze cleanly), interleaved
    # with the three crisp portraits for variety.
    group_pick = [2, 9, 3, 7, 0, 8, 1]
    portraits = ["face_a.jpg", "face_b.jpg", "face_c.jpg"]

    grp = {}
    for gi in group_pick:
        f = faces[gi]
        grp[gi] = (f["box"], f["landmarks"])

    port = {}
    for pn in portraits:
        p = str(fx / pn)
        pdet = run_json(cli, ["detect", "--model", model, "--input", p])
        pf = pdet["faces"][0]
        port[pn] = (Image.open(p).convert("RGB"), pf["box"], pf["landmarks"])

    # carousel order: portrait, group, group, portrait, ...
    order = [
        ("p", "face_a.jpg"), ("g", 2), ("g", 9), ("p", "face_c.jpg"),
        ("g", 3), ("g", 7), ("g", 0), ("p", "face_b.jpg"),
        ("g", 8), ("g", 1),
    ]
    for kind, key in order:
        if kind == "g":
            box, lms = grp[key]
            add_from_crop(mimg, box, lms, "real group photo")
        else:
            pimg, box, lms = port[key]
            add_from_crop(pimg, box, lms, "real portrait")
...
def draw_header(cv, idx, n):
    ...
    fx = font(18, False)
    d.text((bx + bw + bw2 + 16, 40), "·  live face recognition",
           font=fx, fill=DIM)
...
def draw_face_card(cv, subj, cx, cy, alpha, box_t, lm_t, scan_t):
    ...
    cap = subj["label"]
    fc = font(15, False)
    ld.text((tx, ty + THUMB + 18), cap, font=fc, fill=DIMMER + (255,))
    fc2 = font(13, False, mono=True)
    ld.text((tx, ty + THUMB + 42),
            "buffalo_l  .  GGUF  .  CPU", font=fc2, fill=DIMMER + (200,))
...
def draw_reco_panel(cv, subj, rx, ry, rw, reveal_t, hold_frame):
    ...
    if nt > 0:
        ...
        d.text((rx, y), "one binary  .  no Python  .  bit-exact vs InsightFace (<=1px, cosine 1.0)",
...
def draw_end_card(subjects, t):
    ...
    d = ImageDraw.Draw(cv)
    cap = f"recognized {n} / {n} faces, real fixtures, real CLI output"
    ...
    # headline
    t2 = "Live face recognition, one binary, no Python"
    fh = font(40, True)
    t2w = d.textlength(t2, font=fh)
    th2 = clamp(t * 1.3 - 0.2); A2 = int(255 * ease_out(th2))
    d.text(((1280 - t2w) // 2, ly + lh + 50), t2, font=fh, fill=TEAL + (A2,))

    t3 = "detect, align, recognize, age + gender, anti-spoof.  bit-exact vs InsightFace (<=1px, cosine 1.0)"
    fs = font(17, False)
...
def main():
    ...
    ap.add_argument("--out", default=str(HERE.parent / "media"))
    ...
    mp4 = outdir / "face_carousel.mp4"
    gif = outdir / "face_carousel.gif"
