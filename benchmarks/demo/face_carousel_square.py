...
InsightFace pipeline on ~10 REAL faces, real CLI output parsed live), re-laid for
...
from face_carousel import (
    ...
    font, clamp, ease_out, ease_in_out, ease_out_back,
...
def draw_header_sq(cv, idx, n):
    ...
    d.text((bx + bw + bw2 + 14, 42), "·  live face recognition",
...
def draw_face_card_sq(cv, subj, cx, cy, TS, alpha, box_t, lm_t, scan_t):
    ...
    fc_ = font(16, False)
    ld.text((tx, ty + TS + 20), subj["label"], font=fc_, fill=DIMMER + (255,))
    fc2 = font(14, False, mono=True)
    ld.text((tx, ty + TS + 46), "buffalo_l  .  GGUF  .  CPU",
            font=fc2, fill=DIMMER + (200,))
...
def draw_end_card_sq(subjects, t):
    ...
    subs = ["detect, align, recognize, age + gender, anti-spoof.",
            "bit-exact vs InsightFace (<=1px, cosine 1.0)"]
    fs = font(16, False)
...
def main():
    ...
    mp4 = outdir / "face_carousel_square.mp4"
    gif = outdir / "face_carousel_square.gif"
